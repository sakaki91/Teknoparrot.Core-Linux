...
AWL_NAME="Arcade Wrapper Linux/Unix-like" && AWL_VERSION="3.2" && AWL_LOG="/dev/null"
...
case $1 in
    ...
    "--last-dir")
        source src/lastpath
        [[ $2 == "-watch" ]] && echo -e "Last path used: $LASTPATH" && exit
...
FINAL_PATH="$TREE"
mkdir -p src/ && echo LASTPATH=$FINAL_PATH > src/lastpath
hostDependencyChecker
clear
atomicTree
dependencyInstall
